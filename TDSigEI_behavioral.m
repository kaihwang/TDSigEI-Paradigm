function TDSigEI_behavioral(subjname, sessions, motor_mapping)
% Psychtoolbox script for TDSigEI paradigm. This is the behavioral version (not at scanner). Ant can be used
% for practice. The timing thus will be a bit differnt. 
...
for block_conditions = sessions
    ...
    %% Insert a 6 seconds delay after instruction 
    Screen(window,'FillRect',grey);
    Screen(window, 'Flip');
    ...
    for j = 1:block_num
        ...
        if j == block_num
            ...
            Screen('TextSize', window, 80);
            Screen(window, 'Flip');
            DrawFormattedText(window, 'Good Job! Wait for instruction please.', 'center',...
                screenYpixels * 0.5, [0 0 1]);
            Screen(window, 'Flip');
            WaitSecs(4);    
        end
